...
def main():
    ...
    def run_episode(params, u, env):
        ...
        # losses_arr = jnp.zeros(max_episode_length)
        total_loss = 0
        # total_loss = jnp.zeros(1)
        if use_jax_control_flow:
            init_val = (inputs, u, residual, norm_res, losses, norm_resids, 0)

            def cond_fun(args):
                (_, _, _, norm_res, _, _, niters) = args
                return (norm_res > env['restol']
                        and niters < env['max_episode_length'])

            def body_fun(args):
                (inputs, u, residual, norm_res, losses,
                 norm_resids, niters) = args
                ...
                if jnp.isnan(norm_res):
                    print('ERR:', niters)
                    return (u, residual, norm_res, max_episode_length)
            ...
            while (norm_res > env['restol']
                ...
                niters = niters_ if use_full_step_env else niters + 1
                curr_loss = calc_loss(norm_res, niters)

                norm_resids.append(norm_res)
                losses.append(curr_loss)
                # We do this separately in case JAX cannot track through
                # the array
                total_loss += curr_loss

        # return jnp.sum(losses_arr), losses, norm_resids, niters
        # return jnp.sum(jnp.array(losses)), losses, norm_resids, niters
        return total_loss, losses, norm_resids, niters

    def loss(params, u, env):
        total_loss = run_episode(params, u, env)[0]
        return total_loss
    ...
    def update(i, opt_state, u, env):
        params = opt_get_params(opt_state)
        return opt_update(
            i,
            grad_loss(params, u, env),
            opt_state,
        )
    ...
    def train(opt_state):
        ...
        while steps_taken < steps:
            ...
            params = opt_get_params(opt_state)
            episode_loss, losses, norm_resids, niters = run_episode(params, u,
                                                                    env)

            episode_losses.append(episode_loss)
            if steps_taken % collect_data_interval == 0:
                losses.append(losses)
                all_norm_resids.append(norm_resids)
            episode_niters.append(niters)

            opt_state = update(steps_taken, opt_state, u, env)
            # if batch_index >= batch_size:
            #     opt_state = update(steps_taken, opt_state, u, residual, env)
            #     batch_index = 0

            steps_taken += 1
            if steps_taken % 100 == 0:
                num_last_entries = 100
                last_losses = jnp.stack(episode_losses[-num_last_entries:])
                ...
                print(f'Took {steps_taken} steps in '
                      ...
                      f'episode loss: {jnp.mean(last_losses)} '
        ...
        all_norm_resids = jnp.stack(all_norm_resids)
        with open(f'norm_resids_{exp_start_time}.npy', 'wb') as f:
            jnp.save(f, all_norm_resids)

        losses = jnp.stack(losses)
        with open(f'losses_{exp_start_time}.npy', 'wb') as f:
            jnp.save(f, losses)

        episode_niters = jnp.stack(episode_niters)
        with open(f'episode_niters_{exp_start_time}.npy', 'wb') as f:
            jnp.save(f, episode_niters)
